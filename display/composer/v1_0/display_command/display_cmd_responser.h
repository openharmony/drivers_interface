        ...
        bool &needFreeBuffer, bool &needMoveFd, int fd)
    {
        ...
        int32_t ret = layerCache->SetLayerBuffer(data.buffer, data.seqNo, needFreeBuffer, deletingList,
            ...
            if (data.fence > ERROR_FENCE_COUNT || fd > ERROR_FENCE_COUNT || handle.fd > ERROR_FENCE_COUNT) {
                HDF_LOGI("SetLayerBuffer: %{public}s data.devId: %{public}d data.layerId: %{public}d, data.buffer->fd:%{public}d, data.seqNo:%{public}d handle.fd:%{public}d, fd:%{public}d", 
				data.buffer == nullptr ? "data.buffer is nullptr!" : "", data.devId, data.layerId, data.buffer == nullptr ? -1 : data.buffer->fd,
